...
<body class="theme-grey">
	...
	<section>
		<!-- EmailModal -->
		<%@include file="../../admin/ambient/ambientModal.jsp"%>
		<!-- #END# EmailModal -->
	</section>
	...
	<section class="content m-t-90I">
			...
			<div class="row clearfix">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					<div class="" style="padding-top: -5pc;">
						<h2 class="title-Adm m-t-0">AMBIENTES</h2>
					</div>
					<hr>
				</div>
			</div>
						...
						<table class="table table-bordered table-striped table-hover"
							...
							<tbody>
								<c:forEach items="${ambients}" var="ambient">
									<tr id="${ambient.id}">
										<td>${ambient.system.name}</td>
										<td>${ambient.typeAmbient.name}</td>
										<td>${ambient.code}</td>
										<td>${ambient.name}</td>
										<td><div class="iconLineC">
												<a onclick="updateAmbientModal(${ambient.id})"><i
													class="material-icons gris" style="font-size: 30px;">mode_edit</i></a>

												<a onclick="confirmDeleteAmbient(${ambient.id})"><i
		...
		<a id="buttonAddAmbient" type="button"
			class="btn btn-primary btn-fab waves-effect fixedDown"
			onclick="openAmbientModal()"> <i class="material-icons lh-1-8">add</i>
		</a>
	</section>
	...
	<script
		src="<c:url value='/static/js/admin/ambient.js?v=${jsVersion}'/>"></script>
