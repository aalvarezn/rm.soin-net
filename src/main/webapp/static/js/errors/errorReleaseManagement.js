	...
	$dtRFCs = $('#dtRFCs').DataTable(
			{
				...
				lengthMenu : [ [ 10, 25, 50, -1 ],
					...
					"processing" : true,
					"serverSide" : true,
					"sAjaxSource" : getCont() + "management/error/release/list",
					...
					"aoColumns" : [
...

						{
							...
							"mDataProp" : "release.releaseNumber"
						},
						...
						{
							"mRender" : function(data, type, row, meta) {
								return row.release.user.fullName;
							},
						}
						],
						ordering : false,
...
function downLoadReportExcel(){
	...
	$.ajax({
		...
		timeout : 60000,
		data : {
			dateRange :$('#tableFilters input[name="daterange"]').val().replaceAll("/","^"),
			projectId: $('#tableFilters #projectId').children("option:selected").val(),
			errorId: $('#tableFilters #errorId').children("option:selected").val(),
			systemId: $('#tableFilters #systemId').children("option:selected").val()
		},
