...
function confirmCancelRFC(index){
	Swal.fire({
		title: '\u00BFEst\u00e1s seguro que desea cambiar el estado del error a obsoleto?',
...
function cancelRFC(index) {
	...
	$.ajax({
		type : "GET",
		url : getCont() + "baseKnowledge/cancelError",
		timeout : 60000,
		data : {
			idError : index
		},
...
function responseCancelRFC(response) {
	...
	case 'success':
		swal("Correcto!", "El error ha cambiado de estado exitosamente.",
				"success", 2000)
