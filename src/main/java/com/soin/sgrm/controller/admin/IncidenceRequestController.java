...
public class IncidenceRequestController extends BaseController {
	public static final Logger logger = Logger.getLogger(IncidenceRequestController.class);
	...
	public @ResponseBody JsonResponse update(HttpServletRequest request, @RequestBody StatusIncidence uptStatusIncidence) {
		...
		try {
			...
			Sentry.capture(e, "statusIncidence");
			res.setStatus("exception");
			res.setMessage("Error al modificar estado incidencia!");
			logger.log(MyLevel.RELEASE_ERROR, e.toString());
		}
		return res;
	}
	...
	public @ResponseBody JsonResponse delete(@PathVariable Long id, Model model) {
		...
		try {
			...
			res.setStatus("exception");
			res.setMessage("Error al eliminar el estado incidencia!");
			logger.log(MyLevel.RELEASE_ERROR, e.toString());
		}
		return res;
	}
