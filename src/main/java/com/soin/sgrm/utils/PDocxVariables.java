...
public class PDocxVariables {

	Variables variables = null;

	public PDocxVariables() {
	...
	public void generalInfo(PReleaseSummary release, PSystemService systemService) throws Exception {
		PSystemModule module = systemService.findModuleBySystem(release.getSystem().getId());
	...
	public void releaseSolutionInformation(PReleaseSummary release) {
	...
	public void releaseAmbientInformation(PReleaseSummary release) {
	...
	public void releaseInstalationInstructions(PReleaseSummary release) {
	 ...
	 **/
	public void releaseDataBaseInstructions(PReleaseSummary release) {
	 ...
	 **/
	public void releaseObjects(PReleaseSummary release, boolean bd) {
		...
		for (PReleaseObject object : release.getObjects()) {
			if (!bd) {
	 ...
	 **/
	public void releaseActions(PReleaseSummary release) {
		if (release.getActions().size() == 0) {
			...
			for (PReleaseAction action : release.getActions()) {
				ambObservation
	 ...
	 **/
	public void releaseEnvironment(PReleaseSummary release) {
	 ...
	 **/
	public void releaseAmbients(PReleaseSummary release) {
		if (release.getAmbients().size() == 0) {
			...
			for (PAmbient ambient : release.getAmbients()) {
	 ...
	 **/
	public void releaseModifiedComponents(PReleaseSummary release) {
	 ...
	 **/
	public void releaseButtonCommandDetails(PReleaseSummary release) {
		if (release.getButtons().size() > 0) {
			...
			for (PButtonCommand button : release.getButtons()) {
	 ...
	 **/
	public void releaseButtonFileDetails(PReleaseSummary release) {
		if (release.getButtonsFile().size() > 0) {
			...
			for (PButtonFile button : release.getButtonsFile()) {
