...
public final class Sentry {
	...
	/**
	 * A synchronization guard of the stored client. Not using the class as the
	 * guard because that could theoretically deadlock with 3rd party code if it
	 * also synced on the class.
	 */
	...
	/**
	 * Optional override for the default resource loader used to look for
	 * properties.
	 *
	 * @deprecated This was a hack to be able to inject the resource loader into the
	 *             static Lookup initialization. This is no longer required due to
	 *             {@link Lookup} being configurable and passed throughout the
	 *             classes as an instance.
	 */
	@Deprecated
	 ...
	 * <p>
	 * This uses a default lookup instance, use {@link #init(SentryOptions)} if you
	 * need to pass a specially configured lookup.
	 *
	 * @return SentryClient
	 ...
	 *
	 * The canonical way of using this method is: {@link Lookup} lookup = ... obtain
	 * or construct the instance of this class to be able to locate Sentry config
	 * String dsn = ... obtain the Sentry data source name or leave null for lookup
	 * in the configuration SentryClient client =
	 * Sentry.init({@link SentryOptions}.{@link SentryOptions#from(Lookup, String)
	 * from(lookup, dsn))}; If you want to rely on the default mechanisms to obtain
	 * the configuration, you can also use the {@link SentryOptions#defaults()}
	 * method which will use the default way of obtaining the configuration and DSN
	...
	public static SentryClient init(SentryOptions sentryOptions) {
		// Hack to allow Lookup.java access to a different resource locator before its
		...
		// make sure to use the DSN configured in the options instead of the one that
		// the factory can find in its
		// lookup
	 ...
	 * <p>
	 * Note that this method will return true even when {@link SentryClient} was
	 * disabled passing an empty value as {@link Dsn}
	 *
	 ...
	 * instance is already stored, an attempt will be made to create a
	 * {@link SentryClient} from the configuration found in the environment.
	 *
	 ...
	 * @deprecated Using this field is discouraged in favour of using the
	 *             configurable {@link Lookup} with
	 ...
	 */
	public static void capture(Throwable throwable, String module) {
		getStoredClient().clearContext();
		getStoredClient().addTag("module", module);
